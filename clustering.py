...
resume_data = []
# Define the folder path that contains the resumes
folder_path = "C:/Users/marie/OneDrive/Desktop/Scraping/Resumes"
    ...
    if filename.endswith(".pdf"):
        ...
        with pdfplumber.open(os.path.join(folder_path, filename)) as pdf:
            ...
            for page in pdf.pages:
                ...
                print("-" * 50) # underline the fields
            resume_data.append({ 'First Name': first_name, 'Last Name': last_name, 'Email': email, 'Phone': phone, 'Education': education, 'Experience': experience, 'Leadership & Activities': leadership_activities, 'Skills & Interests': skills_interests, 'Personal Information': personal})
            df = pd.DataFrame(resume_data)

            df.to_excel('resume2_data.xlsx', index=False)
...
# Iterate through all the resume data and insert into the Resume table
for resume in resume_data:
    sql = "INSERT INTO Resume (First_Name, Last_Name, Email, Phone, Education, Experience, Leadership_Activities, Skills_Interests, Personal_Info) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
    val = (resume['First Name'], resume['Last Name'], resume['Email'], resume['Phone'], resume['Education'], resume['Experience'], resume['Leadership & Activities'], resume['Skills & Interests'], resume['Personal Information'])
